...
import { nftTransfer } from '@okto_web3/react-sdk'; // [!code highlight]
...
```typescript
import { useOkto } from '@okto_web3/react-sdk';
import { nftTransfer } from '@okto_web3/react-sdk/userop'; // [!code highlight]
...
async function getNFTOrder(jobId: string) {
    ...
    try {
        ...
        const orders = await getOrdersHistory(oktoClient, { // [!code highlight]
            intentId: jobId,                    // Filter by specific order/transaction // [!code highlight]
            intentType: "NFT_TRANSFER",         // Filter for NFT transfer orders // [!code highlight]
            status: "SUCCESSFUL"                // Filter by status: "SUCCESSFUL" | "FAILED" | "PENDING" | "REJECTED" // [!code highlight]
        }); // [!code highlight]
...
// Usage example:
const order = await getNFTOrder(jobId); // [!code highlight]

...
-   **Node.js and npm/pnpm/yarn:**  Make sure you have Node.js and your preferred package manager installed.
-   **Okto API Keys:** You'll need your Okto API keys to connect your app to the Okto platform. Obtain your `NEXT_PUBLIC_VENDOR_PRIVATE_KEY` and `NEXT_PUBLIC_VENDOR_SWA` from the [Okto Dashboard](https://dashboard.okto.tech/login).
-   **Google OAuth Credentials:** To let users log in with Google, you need to set up a Google OAuth application. Go to the [Google Cloud Console](https://console.cloud.google.com/) and create a project and OAuth credentials to get your `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`.
-   **Auth Secret:** NextAuth needs a secret key to keep your app secure. You can easily generate one by running this command in your terminal:  `openssl rand -base64 32`. Copy the output – that's your `AUTH_SECRET`!
...
<Step>
    ## Install the Okto SDK and other dependencies
    ...
    This command installs the necessary packages as dependencies for your project.
...
<Step>
    ...
    Environment variables are like secret keys for your app. They store sensitive information and configurations.  You need to create a `.env.local` file and put your API keys and secrets there.

    1.  **Create `.env.local` file:** In the root of your `my-okto-app` project (the same folder where you ran `create-next-app`), create a new file named exactly `.env.local`.  *(Make sure there's a dot at the beginning and `.local` at the end!)*
    2.  **Add variables:** Open `.env.local` in your code editor and paste in the code below. **Important:** Replace `YOUR_...` placeholders with the actual values you got from the Okto Dashboard and Google Cloud Console in the Prerequisites step!
    ...
    -   **`AUTH_SECRET` Generation:** If you haven't already, run `openssl rand -base64 32` in your terminal to generate a secure `AUTH_SECRET` and paste it into `.env.local`.
...
<Step>
    ...
    const config = { // Configuration for Okto SDK
        environment: process.env.NEXT_PUBLIC_ENVIRONMENT || 'sandbox', // Environment from .env.local
        vendorPrivKey: process.env.NEXT_PUBLIC_VENDOR_PRIVATE_KEY || '', // Vendor Private Key from .env.local
        vendorSWA: process.env.NEXT_PUBLIC_VENDOR_SWA || '', // Vendor SWA from .env.local
    };
    ...
    export const authOptions: AuthOptions = {
      ...
      providers: [
        GoogleProvider({ // Configure Google Provider
          clientId: process.env.GOOGLE_CLIENT_ID!, // Your GOOGLE_CLIENT_ID from .env.local
          clientSecret: process.env.GOOGLE_CLIENT_SECRET!, // Your GOOGLE_CLIENT_SECRET from .env.local
        }),
      ],
      session: {
        strategy: "jwt",
...
If you encounter any errors when running `npm run dev` (or `pnpm dev`, `yarn dev`), check your terminal for error messages. Common issues include:
    -   **Environment Variable Errors**: Double-check that you correctly set up your `.env.local` file and that all the `YOUR_...` placeholders are replaced with your actual keys and secrets.
...
<Step>
 ## Congratulations! You're a Web3 Dapp Developer!

🎉 Congratulations! You've done it! You've successfully set up a Next.js application that's integrated with the Okto SDK and Google Authentication.

**You now have a basic foundation to build your own Web3 dApps!**

**What's next? Explore more!**

-   **Dive Deeper into Okto SDK Features:**  This quick start just scratched the surface. Explore the [Okto SDK documentation](/docs/react-sdk/overview) to discover all the powerful features Okto offers, like portfolio management, transactions, smart contract interactions, and much more!
-   **Check out the Template Repo:** For a more complete example app with more features and UI elements, check out the [Template Repo](https://github.com/okto-hq/okto-sdkv2-nextjs-template-app). It can give you more ideas and code examples to build upon.
-   **Start Building Your Dapp!**  Now that you have the basic setup, you can start building your own unique Web3 application. Think about what cool features you want to add, and use the Okto SDK and this template as your starting point!

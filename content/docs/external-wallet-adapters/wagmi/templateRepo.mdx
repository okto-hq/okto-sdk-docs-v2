...
description: Explore the template repository for integrating Wagmi with Okto.
full: false
---

A comprehensive walkthrough of the Okto Template App designed for seamless Wagmi integration. This template demonstrates core Okto functionality—including authentication, wallet management, and blockchain interactions—so you can quickly get started building with Okto and Wagmi.
...
<div className="bg-white dark:bg-[#1a1a1e] rounded-lg shadow-sm border border-gray-200 dark:border-[#2b2b2c] p-4 w-full my-4">
  <div className="flex items-center justify-between">
    <span className="text-base font-semibold text-gray-900 dark:text-gray-100">Wagmi Okto Template Repository</span>
    ...
    */}
      <a
        href="https://github.com/okto-hq/okto-sdkv2-external-adapter-template/tree/bmac-react-wagmi-okto"
        target="_blank"
        rel="noopener noreferrer"
...
<Tabs items={["React"]}>
  ...
  3. Switch to the `bmac-react-wagmi-okto` branch (this is where the project files are located) and install dependencies:
  ```bash
    git checkout bmac-react-wagmi-okto
    npm install
  ```

  <Callout title="Why switch branches?" icon="info"> The default branch (`main`) is empty. All the source code and configuration files are located in the `bmac-react-wagmi-okto` branch. Be sure to check out this branch before proceeding. </Callout>

...
description: Authenticate users using Okto Auth by sending a JSON-RPC request to the Okto Gateway with the jwt token issued by the client to the user as payload. 
full: false
---
  ...
  "params": [
    {
      "authData": {
        ...
        "provider": "client_jwt"           // Authentication provider
      },
      "sessionData": {

...
Intents are high-level abstractions for on-chain actions. Instead of constructing and managing low-level blockchain transactions, you declare the desired outcome—the *intent*—and the Okto platform handles the execution complexity.

## What are Intents on Okto?

Okto Intents are designed to simplify the developer experience by abstracting away the intricacies of blockchain interactions. When an intent is submitted, the Okto platform is responsible for:

-   **Transaction Assembly:** Translating the high-level intent into a compliant on-chain transaction.
-   **Fee Optimization:** Calculating and applying appropriate gas fees for timely execution.
-   **State Management:** Handling the entire lifecycle of the transaction, from submission to confirmation.
-   **Cross-Chain Execution:** Orchestrating complex workflows for intents that span multiple blockchain networks.
...
1. **Estimate the Intent**: Use the `estimate` endpoint to prepare the intent. You provide the high-level details of your intent, and in return, Okto gives you a complete, pre-populated `UserOperation` object along with gas fee estimations.

2. **Execute the Intent**: Next, you take the `userOps` object from the estimate response, sign it on the client-side using the user's ephemeral session key, and submit it for execution.
...
The payload for the `/estimate` endpoint defines the parameters for the desired swap.
...
```
**Key `details` Fields:**
- `fromChainCaip2Id` & `toChainCaip2Id`: Define the source and destination networks. These can be the same for intra-chain swaps.
...
- `fromChainTokenAmount`: The amount of the source token to be swapped, in its lowest denomination.

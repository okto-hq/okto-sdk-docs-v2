...
The `loginUsingOAuth()` method authenticates users using OAuth providers. It generates a session key pair, creates an authenticate payload, and sends it to the Gateway service. Upon successful authentication, it updates the user session. When auto-login is enabled, the SDK will attempt to reuse the last valid session, eliminating the need for users to log in again.

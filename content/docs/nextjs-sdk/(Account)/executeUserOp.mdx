...
function ExecuteOperation() {
    ...
    async function handleExecuteOperation(signedUserOp) {
        try {
            const result = await oktoClient.executeUserOp(signedUserOp); // [!code highlight]
...
|-----------|------|-------------|-----------|
| `userop` | [`SignedUserOp`](/docs/nextjs-sdk/technical-reference#models) | The signed user operation to be executed | Yes |

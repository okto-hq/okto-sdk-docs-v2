...
import { getPortfolioNFT } from '@okto_web3/react-sdk'; // [!code highlight]
...
function NFTPortfolio() {
    ...
    async function fetchNFTPortfolio() {
        try {
            const nftPortfolio = await getPortfolioNFT(oktoClient); // [!code highlight]
...
|------------|------|-------------|
| `nfts` | [`Promise<UserNFTBalance[]>`](/docs/nextjs-sdk/technical-reference#models) | Array of user's NFT balances |

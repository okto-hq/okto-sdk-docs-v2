...
3. **[Authenticate with Okto](/docs/react-native-sdk/authenticate-users/session-management/loginUsingOAuth)** 
...
Upon successful login, the Google OAuth server generates and sends an `id_token` to your app, confirming the user's identity.

### 3. Client Invokes Okto React Native SDK Authentication
Your app then calls the `loginUsingOAuth` method of the Okto SDK, passing the `id_token` received from Google.
...
For detailed implementation instructions, refer to our [Login Using OAuth guide](/docs/react-native-sdk/authenticate-users/session-management/loginUsingOAuth).    
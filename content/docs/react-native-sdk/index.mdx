...
description: Learn how to create a React Native app and initialize it with the Okto SDK, including setting up authentication and executing your first token transfer.
full: false
---
...
<Callout>
Want an even faster setup? Check out our <ExternalLink href="https://github.com/okto-hq/okto-sdkv2-react-native-template-app">Template Repo</ExternalLink> to get your app running in minutes!
</Callout>
...
<Step>
    ...
    <Tabs items={['npm', 'yarn']}>
        <Tab value="npm">
            ```bash
            npm i @react-native-community/google-signin @react-native-async-storage/async-storage @okto_web3/react-native-sdk@latest
            npx pod-install
            ```
        </Tab>
        <Tab value="yarn">
            ```bash
            yarn add @react-native-community/google-signin @react-native-async-storage/async-storage @okto_web3/react-native-sdk@latest
            npx pod-install
            ```
        </Tab>
    </Tabs>
...
<Step>
    ...
    Create new directory called `components` with a file called `GoogleSignInButton.tsx` in your root folder.
    ...
    export function GoogleSignInButton() {
        ...
        const signIn = async () => {
            try {
                ...
                // Authenticate with Okto using Google token
...
<Step>
    ## 6. Get User Details and Portfolio
...
<Step>
    ...
    Start your app on your preferred platform:
...
<Step>
    ...
    🎉 Your basic Okto integration is now complete! Let's try out a simple user operation!
</Step>
...
Let's implement a token transfer on Solana devnet to understand how user operations work in Okto.
...
<Step>
    ...
    const solanaAccount = accounts.data.find(
        account => account.network_name === "SOLANA_DEVNET"
    );
...
<Step>
    ...
    Add funds to your wallet using the [Solana Devnet Faucet](https://faucet.solana.com).
</Step>
...
<Step>
    ...
    ```typescript title="components/TokenTransfer.tsx"
    import React, { useState } from 'react';
        ...
        async function handleTransfer() {
            try {
                const transferParams = {
                    ...
                    caip2Id: "solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1" // Solana devnet chain ID
                };

...
|------------|------|-------------|
| `hash` | `Promise<string>` | Transaction hash of the executed operation |
...
function ExecuteOperation() {
    ...
    async function handleExecute(signedUserOp) {
        try {
            const txHash = await executeUserOp(oktoClient, signedUserOp);
            console.log('Operation executed:', txHash);
        } catch (error) {
